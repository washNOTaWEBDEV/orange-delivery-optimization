...
- A working list, kept locally, of the 221 localities whose centres fall inside a run outline on the map screenshot, with their census populations (approximate near the outlines; the KML was not used). By population, localities inside only the Warrnambool outline total about 48,300 people, Hamilton about 42,300 (about 27,000 without Horsham, which rarely appears) and Portland about 20,700, with about 800 in localities in overlaps. A further 55 localities lie within about 3 km of an outline, so their run is uncertain. The problem owner says Mortlake is in the Hamilton run and Simpson in the Warrnambool run, and that Macarthur and Hawkesdale may be assigned freely.
- Google Maps, personal experience, and other drivers' experience for geography and timings.
- Population data. The problem owner says population is highly correlated with delivery volume, so it can be treated as a good heuristic for volume for now. The 2021 Census General Community Profile for Victoria has been supplied: total persons by locality (suburbs and localities) are in table G01, column Tot_P_P, for 2,944 localities summing to about 6.49 million. A compact extract (locality name, total persons, area) is kept locally and out of version control. Locality centre coordinates were fetched from public ABS boundary data. In very small localities the count is a weak proxy: one locality on the sample run has no recorded residents. Horsham (15,134 people) is in the Hamilton run but reportedly rarely appears, so the count also overstates its volume. A Victoria population map based on the same census (https://mangomap.com/franchise-demo/maps/88276/Victoria-Population-Map) shows how strongly the three cities dominate the volume.
